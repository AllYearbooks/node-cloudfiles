...
Cloudfiles.prototype.setAuth = function (callback) {
  ...
  request(authOptions, function (err, res, body) {
    ...
    if (self.config.servicenet) {
      console.log("Using servicenet connection.");
      self.config.storageUrl = self.config.storageUrl.replace(/https:\/\//, 'https://snet-');
      console.log(self.config.storageUrl);
    }
...
// Takes a writeable stream and writes the file through, calling back on completion with headers.
Cloudfiles.prototype.streamFile = function (container, filename, lstream, callback) {
  var options = {
    method: 'GET',
    client: this,
    uri: this.storageUrl(container, filename),
    download: lstream
  };
